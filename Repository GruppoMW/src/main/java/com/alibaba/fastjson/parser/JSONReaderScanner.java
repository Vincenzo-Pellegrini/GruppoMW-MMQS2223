...
public final class JSONReaderScanner extends JSONLexerBase {
    ...
    public final char charAt(int index) {
        if (index >= bufLength && bufLength == -1 && index < sp) {
        
            if (bp == 0) {
                char[] buf2 = new char[(this.buf.length * 3) / 2];
                System.arraycopy(this.buf, bp, buf2, 0, bufLength);

                int rest = buf2.length - bufLength;
                extracted4(buf2, rest);
            } else {
    ...
    private void extracted4(char[] buf, int rest){
        try {
    ...
    private void extractedv2(int rest){
        try {
    ...
    private void extracted(int rest) {
        if (rest > 0) {
