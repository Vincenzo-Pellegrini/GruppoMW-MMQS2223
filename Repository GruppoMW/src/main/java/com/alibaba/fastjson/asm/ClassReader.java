...
public class ClassReader {
    ...
    public void accept(final TypeCollector classVisitor) {
        ...
        extracted3(classVisitor, c, anns);
        ...
        i = readUnsignedShort(u);
        u += 2;
        extracted5(classVisitor, c, i, u);
    }

    private int extracted6(int u, int len) {
        int i;
        for (i = 0; i < len; ++i) {
            u += 2;
        }
    ...
    private void extracted5(final TypeCollector classVisitor, char[] c, int i, int u) {
        for (; i > 0; --i) {
    ...
    private int extracted4(int i, int v) {
        int j;
        for (; i > 0; --i) {
    ...
    private void extracted3(final TypeCollector classVisitor, char[] c, int anns) {
        if (anns != 0) {
            extracted2(classVisitor, c, anns);
        }
    }

    private void extracted2(final TypeCollector classVisitor, char[] c, int anns) {
        int i;
        int v;
    ...
    private int extracted(int j, int v) {
        for (; j > 0; --j) {
            v += 6 + readInt(v + 2);
        }
        return v;
    ...
    private int readMethod(TypeCollector classVisitor, char[] c, int u) {
        ...
        if (mv != null && v != 0) {
            ...
            extracted9(c, mv, varTable, varTypeTable);
        }
        return u;
    }

    private int extracted10(int v, int j) {
        for (; j > 0; --j) {
            v += 8;
        }
        return v;
    ...
    private void extracted9(char[] c, MethodCollector mv, int varTable, int varTypeTable) {
        int w;
        int k;
        if (varTable != 0) {
            extracted8(varTypeTable);
    ...
    private void extracted8(int varTypeTable) {
        int w;
        int k;
        if (varTypeTable != 0) {
    ...
    private int extracted7(int u, int v, String attrName) {
    ...
    private int readUnsignedShort(final int index) {
        byte[] b3 = this.b;
        return ((b3[index] & 0xFF) << 8) | (b3[index + 1] & 0xFF);
    }

    private int readInt(final int index) {
        byte[] b1 = this.b;
        return ((b1[index] & 0xFF) << 24) | ((b1[index + 1] & 0xFF) << 16)
                | ((b1[index + 2] & 0xFF) << 8) | (b1[index + 3] & 0xFF);
    }
