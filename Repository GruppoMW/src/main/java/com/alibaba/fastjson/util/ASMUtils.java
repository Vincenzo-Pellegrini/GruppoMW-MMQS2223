...
public class ASMUtils {
    ...
    public static String[] lookupParameterNames(AccessibleObject methodOrCtor) {
        ...
        try {
            ...
            for (int i = 0; i < parameterNames.length; i++) {
                ...
                parameterNames[i] = parameterNamesRefactor(parameterNames[i] , annotations);
            }
    ...
    protected static String parameterNamesRefactor(String parameterNames , Annotation[] annotations){
        String parameterNamesp = parameterNames;
        if (annotations != null) {
            for (int j = 0; j < annotations.length; j++) {
                if (annotations[j] instanceof JSONField) {
                    ...
                    if (fieldName != null && fieldName.length() > 0) {
                        parameterNamesp = fieldName;
                    }
                }
            }
        }
        return parameterNamesp;
    }
}
