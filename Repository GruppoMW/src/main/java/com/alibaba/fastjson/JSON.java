...
public abstract class JSON implements JSONStreamAware, JSONAware {
    ...
    static {
        int features = 0;
        features |= Feature.AUTO_CLOSE_SOURCE.getMask();
        features |= Feature.INTERN_FIELD_NAMES.getMask();
        features |= Feature.USE_BIG_DECIMAL.getMask();
        features |= Feature.ALLOW_UN_QUOTED_FIELD_NAMES.getMask();
        features |= Feature.ALLOW_SINGLE_QUOTES.getMask();
        features |= Feature.ALLOW_ARBITRARY_COMMAS.getMask();
        features |= Feature.SORT_FEID_FAST_MATCH.getMask();
        features |= Feature.IGNORE_NOT_MATCH.getMask();
        DEFAULT_PARSER_FEATURE = features;
    }

    static {
        int features = 0;
        features |= SerializerFeature.QUOTE_FIELD_NAMES.getMask();
        features |= SerializerFeature.SKIP_TRANSIENT_FIELD.getMask();
        features |= SerializerFeature.WRITE_ENUM_USING_NAME.getMask();
        features |= SerializerFeature.SORT_FIELD.getMask();
    ...
    private static void config(Properties properties) {
        extracted8(properties);

        extracted9(properties);

        extracted10(properties);

        extracted11(properties);
    }

    private static void extracted11(Properties properties) {
    ...
    private static void extracted10(Properties properties) {

        if ("true".equals(properties.getProperty("parser.features.ERROR_ON_ENUM_NOT_MATCH"))
                || "true".equals(properties.getProperty("fastjson.parser.features.ERROR_ON_ENUM_NOT_MATCH")))
        {
            //skip
    ...
    private static void extracted9(Properties properties) {
        if ("true".equals(properties.getProperty("parser.features.NON_STRING_KEY_AS_STRING"))) {
            //skip
        }
    }

    private static void extracted8(Properties properties) {
    ...
    public static <T> T parseObject(byte[] bytes, int offset, int len,
        ...
        if (charset == IOUtils.UTF8) {
            ...
            int charsLen = IOUtils.decodeUTF8(bytes, offset, len, chars);

            if (charsLen < 0) {

                try(InputStreamReader gzipReader = new InputStreamReader(new GZIPInputStream(new ByteArrayInputStream(bytes, offset, len)), StandardCharsets.UTF_8)) {
                    strVal = IOUtils.readAll(gzipReader);
                } catch (Exception ex) {
                    return null;
                }
            }
            if (strVal == null && charsLen < 0) {
                return null;
            }

            if (strVal == null) {
                strVal = new String(chars, 0, charsLen);
            }
        } else {
            if (len < 0) {
                return null;
    ...
    public static String toJSONString(Object object, int defaultFeatures, SerializerFeature... features) {
        ...
        try(SerializeWriter out = new SerializeWriter((Writer) null, defaultFeatures, features)) {
            ...
            if (len > 0
                    ...
                    && !out.isEnabled(SerializerFeature.WRITE_CLASS_NAME)) {
    ...
    public static String toJSONString(Object object, boolean prettyFormat) {
        ...
        return toJSONString(object, SerializerFeature.PRETTY_FORMAT);
    }
    ...
    public static void writeJSONString(Writer writer, Object object, int defaultFeatures, SerializerFeature... features) {
        ...
        try(SerializeWriter out = new SerializeWriter(writer, defaultFeatures, features)) {
            JSONSerializer serializer = new JSONSerializer(out);
            serializer.write(object);
        }
    }
    ...
    public static final int writeJSONString(OutputStream os, // 
        ...
        return writeJSONString(os, DEFAULT_GENERATE_FEATURE, features);
    }
    ...
    public static final int writeJSONString(OutputStream os, // 
        ...
        try(SerializeWriter writer = new SerializeWriter(null, defaultFeatures, features)) {
    ...
    public static Object toJSON(Object javaObject, SerializeConfig config) {
        ...
        if (javaObject instanceof Map) {
            ...
            Map innerMap;
            innerMap = extracted(map, size);

            JSONObject json = new JSONObject(innerMap);

            extracted2(config, map, json);
        ...
        if (javaObject instanceof Collection) {
            ...
            extracted3(config, collection, array);
        ...
        if (clazz.isArray()) {
            ...
            extracted4(javaObject, len, array);
        ...
        if (serializer instanceof JavaBeanSerializer) {
            ...
            ordered = extracted6(jsonType, ordered);

            JSONObject json = new JSONObject(ordered);
            extracted7(javaObject, config, javaBeanSerializer, json);
            return json;
        }
    ...
    private static void extracted7(Object javaObject, SerializeConfig config, JavaBeanSerializer javaBeanSerializer,
    ...
    private static boolean extracted6(JSONType jsonType, boolean ordered) {
        if (jsonType != null) {
            for (SerializerFeature serializerFeature : jsonType.serialzeFeatures()) {
                ordered = extracted5(ordered, serializerFeature);
            }
        }
        return ordered;
    }

    private static boolean extracted5(boolean ordered, SerializerFeature serializerFeature) {
        if (serializerFeature == SerializerFeature.SORT_FIELD
                || serializerFeature == SerializerFeature.MAP_SORT_FIELD) {
            ordered = true;
        }
        return ordered;
    }

    private static void extracted4(Object javaObject, int len, JSONArray array) {
        for (int i = 0; i < len; ++i) {
    ...
    private static void extracted3(SerializeConfig config, Collection<Object> collection, JSONArray array) {
    ...
    private static void extracted2(SerializeConfig config, Map<Object, Object> map, JSONObject json) {
    ...
    private static Map<Object, Object> extracted(Map<Object, Object> map, int size) {
     ...
     * @since 1.2.33
     */
    public <T> T toJavaObject(TypeReference<Object> typeReference) {
